...
contract AlignmentVault is Ownable, Initializable, ERC721Holder, ERC1155Holder, IAlignmentVault {
    ...
    function inventoryPositionCreateVToken(uint256 vTokenAmount) external payable virtual onlyOwner {
        _inventoryPositionIds.add(_NFTX_INVENTORY_STAKING.deposit(
            vaultId,
            vTokenAmount,
            address(this),
            "",
    ...
    function inventoryPositionCreateNfts(uint256[] calldata tokenIds, uint256[] calldata amounts) external payable virtual onlyOwner {
        _inventoryPositionIds.add(_NFTX_INVENTORY_STAKING.depositWithNFT(vaultId, tokenIds, amounts, address(this)));
    }
    ...
    function inventoryPositionCollectAllFees() external payable virtual onlyOwner {
        _NFTX_INVENTORY_STAKING.collectWethFees(_inventoryPositionIds.values());
    }
    ...
    function liquidityPositionCreate(uint256 ethAmount, uint256 vTokenAmount, uint256[] calldata tokenIds, uint256[] calldata amounts, int24 tickLower, int24 tickUpper, uint24 fee, uint160 sqrtPriceX96, uint16 slippage) external payable virtual onlyOwner {
        // uint256 vTokensPlusNfts is omitted here due to stack too deep
        uint256 vTokenAmountMin;
        uint256 wethMin;
        unchecked {
            vTokenAmountMin = (vTokenAmount + (tokenIds.length * 10e18)) - FixedPointMathLib.fullMulDiv(vTokenAmount + (tokenIds.length * 10e18), slippage, _SLIPPAGE_DENOMINATOR);
    ...
    function liquidityPositionIncrease(uint256 positionId, uint256 ethAmount, uint256 vTokenAmount, uint256[] calldata tokenIds, uint256[] calldata amounts, uint16 slippage) external payable virtual onlyOwner {
        uint256 vTokensPlusNfts;
        uint256 vTokenAmountMin;
        uint256 wethMin;
        unchecked {
            vTokensPlusNfts = vTokenAmount + (tokenIds.length * 10e18);
            vTokenAmountMin = vTokensPlusNfts - FixedPointMathLib.fullMulDiv(vTokensPlusNfts, slippage, _SLIPPAGE_DENOMINATOR);
            wethMin = ethAmount - FixedPointMathLib.fullMulDiv(ethAmount, slippage, _SLIPPAGE_DENOMINATOR);
        }
    ...
    function liquidityPositionCollectFees(uint256 positionId) external payable virtual onlyOwner {
    ...
    function buyNftFromPool(uint256 ethAmount, uint256[] calldata tokenIds, uint256 vTokenPremiumLimit, uint24 fee, uint160 sqrtPriceLimitX96) external payable virtual onlyOwner {
        if (vTokenPremiumLimit == 0) vTokenPremiumLimit = type(uint256).max;
