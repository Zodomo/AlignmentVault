...
interface IAlignmentVault {
    ...
    event AV_NftsPurchased(
        uint256 indexed ethAmount,
        uint256[] indexed tokenIds
    );
    event AV_MintVTokens(uint256[] indexed tokenIds, uint256[] indexed amounts);
    event AV_DelegateSet(
        address indexed oldDelegate,
        address indexed newDelegate
    );

    // Inventory Management Events
    event AV_InventoryPositionCreated(
        uint256 indexed positionId,
        uint256 indexed vTokenAmount
    );
    event AV_InventoryPositionIncreased(
        uint256 indexed positionId,
        uint256 indexed vTokenAmount
    );
    event AV_InventoryPositionWithdrawal(
        uint256 indexed positionId,
        uint256 indexed vTokenAmount
    );
    event AV_InventoryPositionCombination(
        uint256 indexed positionId,
        uint256[] indexed childPositionIds
    );
    ...
    event AV_LiquidityPositionCombination(
        uint256 indexed positionId,
        uint256[] indexed childPositionIds
    );
    ...
    // View Functions
    function getUniswapPoolValues()
        external
        view
        returns (address pool, uint160 sqrtPriceX96, int24 tick);
    function getInventoryPositionIds()
        external
        view
        returns (uint256[] memory positionIds);
    function getLiquidityPositionIds()
        external
        view
        returns (uint256[] memory positionIds);
    ...
    function getTotalInventoryPositionFees()
        external
        view
    ...
    function getTotalLiquidityPositionFees()
        external
        view
    ...
    function inventoryPositionIncrease(
        uint256 positionId,
        uint256 vTokenAmount
    ) external payable;
